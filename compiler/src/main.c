...
static void print_usage(void) {
    printf(
        "Forge Compiler v" FORGE_VERSION "\n\n"
        "Usage:\n"
...
static char *read_file(const char *path) {
    ...
    if (!f) { fprintf(stderr, "forge: cannot open '%s'\n", path); return NULL; }
    fseek(f, 0, SEEK_END);
    long sz = ftell(f);
    rewind(f);
...
static int compile_file(const char *path, const CompileConfig *cfg) {
    ...
    if (!cfg->no_wasm) {
        ...
        if (!wasm_check_toolchain(&w_opts)) {
            ...
            for (int i = 0; i < prog->component_count; i++) {
                char c_path[512];
                snprintf(c_path, sizeof(c_path), "%s/%s.gen.c",
                         cfg->out_dir, prog->components[i]->name);

                WasmResult wr = wasm_compile(c_path, &w_opts);
                if (wr.success) {
                    printf("forge: \033[32m✓\033[0m %s  (%zu bytes)\n",
                           wr.wasm_path, wr.wasm_size);
                } else {
    ...
    for (int i = 0; i < prog->component_count; i++) {
        ...
        if (!cfg->no_types) {
            char dts_path[512];
            snprintf(dts_path, sizeof(dts_path), "%s/%s.forge.d.ts", cfg->out_dir, c->name);
