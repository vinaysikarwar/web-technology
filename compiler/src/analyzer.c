...
static void ana_error(AnalyzerCtx *ctx, const char *msg) {
  fprintf(stderr,
          "\033[31m[forge/analyzer] ERROR\033[0m  in component '%s': %s\n",
          ctx->comp->name, msg);
  ctx->errors++;
}
...
static void ana_warn(AnalyzerCtx *ctx, const char *msg) {
  fprintf(stderr,
          "\033[33m[forge/analyzer] WARN\033[0m   in component '%s': %s\n",
          ctx->comp->name, msg);
  ctx->warnings++;
}
...
static void check_unused(AnalyzerCtx *ctx) {
  ...
  for (int i = 0; i < c->state_count; i++) {
    if (!c->state_used_in_template[i]) {
      ...
      snprintf(msg, sizeof(msg),
               "state field '%s' is declared but never used in @template or "
               "@on handlers",
  ...
  for (int i = 0; i < c->prop_count; i++) {
    if (!c->props_used_in_template[i]) {
      char msg[256];
      snprintf(msg, sizeof(msg), "prop '%s' is declared but never used",
...
static void check_template(AnalyzerCtx *ctx) {
  if (!ctx->comp->template_root) {
    ana_error(ctx, "Component has no @template section — every component must "
                   "render something");
  }
}
...
AnalysisResult analyze_component(ComponentNode *c) {
  ...
  for (int i = 0; i < c->style_count; i++) {
    if (strstr(c->style[i].value, "props.") ||
        strstr(c->style[i].value, "state.")) {
